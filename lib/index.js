      ...
      getPropsFromModule(layoutModule)
        .then(layoutProps => Promise.all(this.options.pages.map(page => {
          ...
          return getPropsFromModule(pageModule)
            ...
            //modify the props
            .then(pageProps => Promise.all([

              Promise.resolve(this.options.getLayoutProps(layoutProps, {
                pageChunk: pageChunk,
                layoutChunk: layoutChunk,
                compilation
              })),

              Promise.resolve(this.options.getPageProps(pageProps, {
                pageChunk: pageChunk,
                layoutChunk: layoutChunk,
                compilation
              }))
            ...
            //render the page
            .then(modifiedProps => {
              const modifiedLayoutProps = modifiedProps[0];
              const modifiedPageProps = modifiedProps[1];

              if (Array.isArray(modifiedPageProps)) {

                return Promise.all(modifiedPageProps.map(props => {
                  return renderPage(layoutModule, modifiedLayoutProps, pageModule, props)
                ...
                return renderPage(layoutModule, modifiedLayoutProps, pageModule, modifiedPageProps)
